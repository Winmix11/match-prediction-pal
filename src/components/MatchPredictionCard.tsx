...
import { getMatchAnalysis, calculateOdds, formatProbability } from '@/lib/api';
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <img src={homeTeam.logo} alt={homeTeam.name} className="h-6 w-6" />
                <span className="font-medium">{homeTeam.name}</span>
              </div>
              <span>vs</span>
              <div className="flex items-center gap-2">
                <span className="font-medium">{awayTeam.name}</span>
                <img src={awayTeam.logo} alt={awayTeam.name} className="h-6 w-6" />
              </div>
            </div>
              ...
              <div className="p-3 bg-muted/30 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm">
                    {analysis.prediction.predictedWinner === 'home' ? homeTeam.name : 
                     analysis.prediction.predictedWinner === 'away' ? awayTeam.name : 'Draw'}
                  </span>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-muted/30 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm">
                    {analysis.prediction.bothTeamsToScoreProb > 50 ? 'Yes' : 'No'}
                  </span>
                  <span className="text-xs text-blue-400">
                    {analysis.prediction.bothTeamsToScoreProb.toFixed(0)}%
                  </span>
                </div>
